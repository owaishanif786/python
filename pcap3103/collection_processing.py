...
name_declarations = list(map(lambda p: f"{p['name']} is {p['age']} years old", sorted_by_name))
...
print(list(filter( lambda p: p['age'] < 70, sorted_by_name)))
under_seventy =  sorted(filter( lambda p: p['age'] < 70, sorted_by_name), key= lambda p: p['age'])
